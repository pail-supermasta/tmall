...
function destructResponse($res, $order, $shop)
{
    ...
    $orderDetails['memo'] = $shortener['memo'] != "" ? $shortener['memo'] : false;
...
//        error_log("Caught $e in order $order \n Product list $products");
    ...
    $orderDetails['escrow_fee_rates'] = str_replace(['%5D', '%5B'], "", http_build_query($positionsEscrowFee, ' ', ','));
...
// Get order details
function assembleOrderDetails($order, $filename, $countQueries)
{
...
//        'method' => 'aliexpress.trade.new.redefining.findorderbyid',
    ...
    $countQueries = isset($res) ? $countQueries += 1 : $countQueries;
...
//                $link = MS_PATH . "/entity/customerorder/?search=" . $order;
                    ...
                    if ($res) {
                        ...
                        if (!$orderSearchResultArrays['rows']) {
                            ...
                            $getOrderDetailsCount = assembleOrderDetails($order, $filename, $getOrderDetailsCount);
            ...
            /*count aliexpress.solution.order.info.get calls foreach $order in $split in $files*/
//            $getOrderDetailsCount = count($split) - 1;
            $offsetGMTRU = 3 * 60 * 60;
            $logDate = date("Y-m-d H:i:s", strtotime(gmdate("Y-m-d H:i:s")) + $offsetGMTRU);
            $messageQueryCalls = "$getOrderDetailsCount times called from file-processing.php due $filename  at $logDate";
//            countQuery($getOrderDetailsCount, $messageQueryCalls);
        }
